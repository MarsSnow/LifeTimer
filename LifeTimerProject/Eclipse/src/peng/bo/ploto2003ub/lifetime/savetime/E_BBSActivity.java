...
import peng.bo.ploto2003ub.lifetime.common.SIMCardInfo;
...
public class E_BBSActivity extends Activity {
	String value;
	String FirstString = "first";
	Context ThisContext = E_BBSActivity.this;
	...
	public void onCreate(Bundle bundle) {
		...
		object = localIntent.getStringExtra("ObjectName");
...
//		 value = AppConnect.getInstance(this).getConfig_Sync(
//				"timelifeipfilter");
...
//		}
	...
	private void GetPhoneInfo() {
		...
		ProvidersNameString = siminfo.getProvidersName();// 运营商
		NativePhoneNumberString = siminfo.getNativePhoneNumber();//
	...
	private static File createSampleFile() {
		try {
			...
			// writer.append("用户："+ProvidersNameString + ",IMSI码:" +
			// NativePhoneNumberString
			// + "\n" + PhoneVersionString + "\n" + "IP：" + hostip+ "\n" + "【" +
			// shoumingtemp + "】" + "\n"
			// + datetimestr
			// + "\n");
	...
	class PageTask extends
		...
		public PageTask(Context context) {
			...
			// 表明调用的是不是系统的，而是自定义的
			// 设置自定义的风格
			mProgressDialogcircle = new Dialog(ThisContext,
					R.style.theme_dialog_alert);
			// 设置自定义进度条的内容
			mProgressDialogcircle.setContentView(R.layout.window_layout);
			mProgressDialogcircle.setCancelable(true);
			// 设置进度条完成时的情形
			mProgressDialogcircle
		...
		protected void onPostExecute(List<Map<String, Object>> result) {
			...
			if (mProgressDialogcircle != null) {
				mProgressDialogcircle.dismiss();
			}
